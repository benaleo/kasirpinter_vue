    ...
    <Popover v-model:open="open">
      ...
      <PopoverContent class="w-[--radix-popover-trigger-width] p-0">
        <Command :filter-function="customFilter">
          <CommandInput :placeholder="searchPlaceholder || placeholder" class="h-9" />
          <CommandList>
...
const validData = computed(() => {
  ...
  return data.filter(
    (item) => item && typeof item === 'object' && 'value' in item && 'label' in item,
  )
})

const displayValue = computed(() => {
  if (!internalValue.value) return props.placeholder
  const selected = validData.value.find(
    (item) => String(item.value) === String(internalValue.value), // Compare as strings
  )
