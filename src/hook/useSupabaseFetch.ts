...
type FetchOptions = {
  table: string
  columns?: string
  select?: string // Tambahkan opsi select untuk join
  filter?: {
    column: string
    operator: string
    value: any
...
export function useSupabaseFetch<T>(options: FetchOptions) {
  ...
  const executeQuery = async () => {
    try {
      ...
      // Gunakan options.select jika ada, jika tidak gunakan options.columns atau '*'
      const selectQuery = options.select || options.columns || '*'

      let query = supabase
        .from(options.table)
        .select(selectQuery, options.filter ? { count: 'exact' } : undefined)
